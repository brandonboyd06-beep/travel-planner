        ...
        <div className="day-editor-list">
          ...
          {initialDay.stops.map((stop, index) => (
            <article className="day-editor-stop" key={stop.id}>
              ...
              <label><span className="sr-only">Move {stop.name} to another day</span><select value={initialDay.id} onChange={(event) => moveStop(initialDay.id, event.target.value, stop.id)} disabled={!canEdit || stop.priority === 'fixed'}>{plan.days.map((day) => <option value={day.id} key={day.id}>{day.day} {day.date}</option>)}</select></label>
              <div className="stop-row-actions">
                <button type="button" onClick={() => reorderStop(initialDay.id, stop.id, -1)} disabled={!canEdit || stop.priority === 'fixed' || index === 0} aria-label={`Move ${stop.name} earlier`}><ArrowUp /></button>
                <button type="button" onClick={() => reorderStop(initialDay.id, stop.id, 1)} disabled={!canEdit || stop.priority === 'fixed' || index === initialDay.stops.length - 1} aria-label={`Move ${stop.name} later`}><ArrowDown /></button>
                <button type="button" onClick={() => startEditing(initialDay.id, stop)} disabled={!canEdit} aria-label={`Edit ${stop.name}`}><Pencil /></button>
                <button type="button" onClick={() => { if (window.confirm(`Remove ${stop.name} from the itinerary?`)) removeStop(initialDay.id, stop.id) }} disabled={!canEdit || stop.priority === 'fixed'} aria-label={`Remove ${stop.name}`}><Trash2 /></button>
              </div>
            </article>
          ))}
        </div>
