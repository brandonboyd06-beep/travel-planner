  ...
  return (
    <article className="itinerary-day-card">
      ...
      <div className="day-plan">
        ...
        <button className="optional-toggle" type="button" onClick={onToggle} aria-expanded={optionalOn}><Circle size={13} />{optionalOn ? 'See fewer options' : `See more options (${item.optional.length})`}</button>
      ...
      <details className="day-details"><summary>See full day details</summary><div><p><strong>Logistics:</strong> {item.logistics}</p><p><CloudRain size={14} /><strong>Weather backup:</strong> {item.backup}</p><p><strong>Dining:</strong> {item.dining.join(' · ')}</p></div></details>
    </article>
  )
}
