...
export function DiningPage() {
  ...
  return (
    <>
      ...
      <div className="filter-bar compact" role="tablist" aria-label="Restaurant town">{towns.map((item) => <button role="tab" aria-selected={town === item} className={town === item ? 'active' : ''} key={item} onClick={() => setTown(item)}>{item}</button>)}</div>
